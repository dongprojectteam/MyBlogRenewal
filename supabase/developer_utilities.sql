with utility_rows (id, title, description, url, image_url, category, visible, sort_order) as (
  values
    (
...
select source.id, source.title, source.description, source.url, source.image_url, source.category, source.visible, source.sort_order
from utility_rows as source
where not exists (
  select 1
  from updated
